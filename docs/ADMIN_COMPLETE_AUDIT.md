...
now here if user try to update status to pickuped up then we need to open dialog we need to show order details about jewller and pure gold which we already added here we need to add thing manufacture commission same like our commision user will add manufacruer commsion and manufacturer bill number and we need to calculate manufacturer total charges of that jwellery ie if 500 gms then if commision means making charge is 1 then need to show 500 usd we payable to manufacturer and we need to generate recipt if we pay directly then need to select cash and not paying need to select credit entry (u knows the best credit or debit which option we need to show for purchase entry) cash/credit or cash/debit u knows the best I dont know much about accounting got it??? and here gold in transit of that manufacturer will be reduced based on jewllery grams, here we need to show all details about challan if manufacture recived gold multiple time and we need to calculate total pure gold and then based on it we need to increase total grams keep in mind its karat ie initally 150 pure gold and 200 gms jwellery with 18k and if manufacturer again took 50gms then how much weight increased based on its karrrate 18 got it? means final finished product weight increased so based on it we need to show on popup full details about finished product and here gold in transist of that manufacurer , account of that manufactuer and finished good account will be updated because we need to pay to that manufacturer here finished good account will receive pure gold got it u knows the best how to logged accounting entries for this case