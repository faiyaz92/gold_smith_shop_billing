...
export default function GoldSmithOrders() {
  ...
  const [selectedAccountType, setSelectedAccountType] = useState('goldInHand');
  const [selectedGoldBankId, setSelectedGoldBankId] = useState('');
  ...
  // ✅ VALIDATION: Helper function to validate accounts before creating entries
  const validateAccountsForEntry = async (entries) => {
    const accountManager = new HierarchicalAccountManager(companyId || 'default-company');
    
    for (const entry of entries) {
      const isParent = await accountManager.isParentAccount(entry.accountCode);
      if (isParent) {
        alert(`❌ Cannot use parent account ${entry.accountCode} for ${entry.debit > 0 ? 'debit' : 'credit'}. Please use a child account.`);
        return false;
      }
    }
    return true;
  ...
  const processManufacturerPayment = async (order, manufacturerData, commissionAmount, paymentMethod, manufacturerBillNumber, notes) => {
    try {
      ...
      let manufacturerAccountCode = manufacturerData.accountCode;
      if (!manufacturerAccountCode) {
        ...
        // Update local data
        manufacturerData.accountCode = manufacturerAccountCode;
      }
      ...
      if (paymentMethod === 'credit') {
        ...
        const entries = [
          ...
          {
            accountCode: manufacturerAccountCode, // Manufacturer-Specific Payables Account
  ...
  const createAdditionalGoldChallan = async (order, manufacturerData, additionalGoldAmount) => {
    try {
      ...
      if (order.goldBankId) {
        ...
        if (goldBankSnap.exists()) {
          ...
          if (!goldBankAccountCode) {
            ...
            // Update local data
            goldBankData.accountCode = goldBankAccountCode;
          }
        }
      }
      ...
      const goldTransitAccountName = `${manufacturerData.manufacturerName || manufacturerData.name} - Gold in Transit`;
  ...
  const handleShowDeliveryDialog = async (order) => {
    try {
      // ✅ RESTRICTION: If gold challan was issued, pickup must be completed first
      if (order.goldIssued && order.status !== 'Picked Up') {
        alert('Cannot generate invoice: Gold challan was issued for this order. You must complete the pickup process first to record manufacturer entries and gold balance.');
        return;
      }
  ...
  const handleDownloadInvoice = async (order) => {
    try {
      ...
      // Get invoice data
      const invoiceRef = doc(db, `${basePath}/invoices`, order.invoiceId);
      ...
      // Generate and download PDF
      await invoiceGenerator.downloadInvoice(invoiceData, order, customerData);
      ...
      try {
        ...
        if (challanType === 'additional_gold') {
          ...
          const entries = [
            ...
            {
              accountCode: goldBankAccountCode, // ✅ FIXED: Specific gold bank account (never parent)
              accountName: goldBankAccountName,
              debit: 0,
              credit: pureGoldAmount,
              balanceType: 'gold'
            }
          ...
          // Gold Return: Debit Selected Gold Bank, Credit Gold in Transit
          // Get the selected gold bank data to determine the account code
          let goldBankAccountCode = null;
          let goldBankAccountName = 'Gold Bank (Sharaf)';
          ...
          const entries = [
            {
              accountCode: goldBankAccountCode, // ✅ FIXED: Specific gold bank account
              accountName: goldBankAccountName,
              debit: pureGoldAmount,
              credit: 0,
              balanceType: 'gold'
  ...
  const handleGenerateInvoice = async (order) => {
    try {
      ...
      // Debit: Customer Receivable (CUST-XXXX) - NEVER use parent '1301'
      ...
      try {
        ...
        const entries = [
          {
            accountCode: customerAccountCode, // Specific customer account (never parent)
            accountName: `${customerData.customerName || customerData.name} - Receivables`,
            debit: totalPureGold,
            credit: 0,
            balanceType: 'gold'
  ...
  const handleRecordPayment = async () => {
    try {
      ...
      try {
        ...
        // Gold portion: Debit Gold Bank (specific), Credit Customer Receivable (specific)
        if (goldPortion > 0) {
          entries.push({
          ...
          entries.push({
            accountCode: customerAccountCode, // ✅ FIXED: Use specific customer account
            accountName: `${paymentInvoiceData.customerName} - Receivables`,
            debit: 0,
            credit: goldPortion,
            balanceType: 'gold'
          });
        ...
        // USD portion: Debit Cash (1201), Credit Customer Receivable (specific)
        if (usdPortion > 0) {
          entries.push({
            accountCode: '1201',
      ...
      if (newPaymentStatus === 'paid') {
        ...
        // Update remaining balance on order for partial payments
        try {
          const orderRef = doc(db, ordersPath, paymentInvoiceData.orderId);
          await updateDoc(orderRef, {
            paymentStatus: newPaymentStatus === 'partially_paid' ? 'Partially Paid' : 'Not Paid',
            remainingPureGold: newRemainingAmount,
            updatedAt: serverTimestamp()
          });
        } catch (orderError) {
  ...
  // ✅ Handle Quick Customer Payment (USD to Gold Only)
  const handleQuickCustomerPayment = async (order) => {
    try {
      // Fetch invoice data
      const invoicesPath = `${basePath}/invoices`;
      ...
      if (!invoiceSnap.empty) {
        ...
        const defaultGoldPrice = currentGoldPrice?.pricePerGram || 145.43;
        setQuickPaymentData({
          usdAmount: '',
          goldPrice: defaultGoldPrice.toFixed(2),
          notes: ''
        });
      ...
      console.error('Error preparing quick payment:', error);
      alert('Error preparing payment dialog');
    }
  };

  // ✅ Process Quick Customer Payment
  const handleConfirmQuickPayment = async () => {
    try {
      const { usdAmount, goldPrice, notes } = quickPaymentData;
      const order = quickPaymentOrder;
      ...
      const usdAmountValue = parseFloat(usdAmount);
      const goldPriceValue = parseFloat(goldPrice);
      const goldEquivalent = usdAmountValue / goldPriceValue;
      
      // Validate payment doesn't exceed remaining balance
      const remainingBalance = order.remainingPureGold || order.invoiceData?.remainingPureGold || 0;
      if (goldEquivalent > remainingBalance) {
        alert(`Payment amount (${goldEquivalent.toFixed(3)}g) exceeds remaining balance (${remainingBalance.toFixed(3)}g). Please adjust the amount.`);
        return;
      }

      // ✅ CHECK: Ensure default accounts are initialized
      const accountsInitialized = await checkAccountsInitialized();
      if (!accountsInitialized) {
        alert('❌ Default accounts not initialized. Please initialize accounts from the Accounting page first.');
        return;
      ...
      // Fetch invoice data again to ensure current state
      const invoicesPath = `${basePath}/invoices`;
      const invoiceRef = doc(db, invoicesPath, order.invoiceId);
      const invoiceSnap = await getDoc(invoiceRef);
      ...
      const invoiceData = invoiceSnap.data();
      const currentRemainingBalance = invoiceData.remainingPureGold || 0;

      // Double-check validation with current data
      if (goldEquivalent > currentRemainingBalance) {
        alert(`Payment amount (${goldEquivalent.toFixed(3)}g) exceeds current remaining balance (${currentRemainingBalance.toFixed(3)}g). Please refresh and try again.`);
        return;
      }
      ...
      const paymentDoc = {
        ...
        pureGoldPaid: goldEquivalent,
        goldPriceAtPayment: goldPriceValue,
        displayAmountUSD: usdAmountValue,
        paymentMethod: 'usd_cash',
        paymentMode: 'usd_cash',
        goldPortion: 0,
        usdPortion: usdAmountValue,
        usdEquivalentGold: goldEquivalent,
        priceAdjusted: false,
        previousBalance: currentRemainingBalance,
        amountPaid: goldEquivalent,
        newBalance: currentRemainingBalance - goldEquivalent,
        depositedToSharaf: false,
        accountingRecorded: false,
        status: 'completed',
        paymentDate: serverTimestamp(),
      ...
      // Save payment to Firestore
      const paymentRef = await addDoc(collection(db, paymentsPath), paymentDoc);
      console.log('Quick payment created:', paymentRef.id);

      // Update invoice
      const newPaidAmount = (invoiceData.paidPureGold || 0) + goldEquivalent;
      const newRemainingAmount = invoiceData.totalPureGold - newPaidAmount;
      const newPaymentStatus = newRemainingAmount <= 0 ? 'paid' : 
        newPaidAmount > 0 ? 'partially_paid' : 'not_paid';
      ...
      // Create accounting entries (USD to Gold conversion)
      try {
        const accountingEngine = new AccountingEngine(companyId || 'default-company');
        ...
        await accountingEngine.createEntry({
          date: new Date(),
          description: `Quick Payment ${paymentNumber} received from ${order.customerName} for Invoice ${order.invoiceNumber} - ${goldEquivalent.toFixed(3)}g pure gold equivalent`,
          transactionType: 'customer_payment',
          referenceId: paymentRef.id,
          referenceType: 'payment',
          entries: [
            {
              accountCode: '1201', // Cash in Hand (USD)
              accountName: 'Cash in Hand',
              debit: usdAmountValue,
              credit: 0,
              balanceType: 'usd'
            },
            {
              accountCode: customerAccountCode, // Specific customer account (never parent)
              accountName: `${order.customerName} - Receivables`,
              debit: 0,
        ...
        console.log('Accounting entry created for quick payment');
      } catch (accountingError) {
        console.error('Error creating accounting entry:', accountingError);
        // Continue - payment is still recorded
      }
      ...
      try {
        ...
        if (customerSnap.exists()) {
          const customerData = customerSnap.data();
          const newCustomerBalance = (customerData.currentPureGoldBalance || 0) - goldEquivalent;
          const newTotalPaid = (customerData.totalPureGoldPaid || 0) + goldEquivalent;
      ...
      alert(`Payment recorded successfully!\nAmount: $${usdAmountValue.toFixed(2)} (${goldEquivalent.toFixed(3)}g pure gold)\nRemaining Balance: ${newRemainingAmount.toFixed(3)}g`);
      fetchInitialData();
  ...
  const handleConfirmRecordPayment = async () => {
    try {
      ...
      if (selectedAccountType === 'goldBank' && !selectedGoldBankId) {
        alert('Please select a gold bank');
        return;
      }

      // Validate payment doesn't exceed remaining balance
      const remainingBalance = order.remainingPureGold || 0;
      if (amount > remainingBalance) {
        alert(`Payment amount (${amount.toFixed(3)}g) exceeds remaining balance (${remainingBalance.toFixed(3)}g). Please adjust the amount.`);
        return;
      }
      ...
      if (amount > currentRemainingBalance) {
        alert(`Payment amount (${amount.toFixed(3)}g) exceeds current remaining balance (${currentRemainingBalance.toFixed(3)}g). Please refresh and try again.`);
        return;
      }
      ...
      if (selectedAccountType === 'goldInHand') {
        debitAccountCode = '1104'; // Gold in Hand
        ...
        if (goldBankSnap.exists()) {
          const goldBankData = goldBankSnap.data();
          debitAccountCode = goldBankData.accountCode;
          if (!debitAccountCode) {
            // Create gold bank account if missing
            const accountManager = new HierarchicalAccountManager(companyId);
          ...
          debitAccountName = `${goldBankData.bankName || 'Gold Bank'} - Gold Custody`;
        } else {
          alert('Selected gold bank not found.');
          return;
        }
      }
      
      // ✅ ACCOUNT CODE AUDIT: Ensure customer has account code
      let creditAccountCode = order.customerData?.accountCode;
      if (!creditAccountCode) {
        ...
        // Update local data
        order.customerData.accountCode = creditAccountCode;
      }
      ...
      const paymentDoc = {
        ...
        customerName: order.customerName,
        pureGoldPaid: amount,
        paymentMethod: selectedAccountType === 'goldInHand' ? 'gold_in_hand' : 'gold_bank',
        paymentMode: selectedAccountType === 'goldInHand' ? 'gold_in_hand' : 'gold_bank',
        goldPortion: amount,
        usdPortion: 0,
        usdEquivalentGold: 0,
        priceAdjusted: false,
        previousBalance: currentRemainingBalance,
        amountPaid: amount,
        newBalance: currentRemainingBalance - amount,
        depositedToSharaf: selectedAccountType === 'goldBank',
        accountingRecorded: false,
        status: 'completed',
        paymentDate: serverTimestamp(),
      ...
      // Update invoice
      const newPaidAmount = (invoiceData.paidPureGold || 0) + amount;
      const newRemainingAmount = invoiceData.totalPureGold - newPaidAmount;
      const newPaymentStatus = newRemainingAmount <= 0 ? 'paid' : 
        newPaidAmount > 0 ? 'partially_paid' : 'not_paid';
      ...
      // Create accounting entries
      try {
        // ✅ VALIDATION: Check for parent account usage
        const entries = [
          {
            accountCode: debitAccountCode,
        ...
        await accountingEngine.createEntry({
          date: new Date(),
          description: `Payment ${paymentNumber} received from ${order.customerName} for Invoice ${order.invoiceNumber} - ${amount.toFixed(3)}g pure gold`,
          transactionType: 'customer_payment',
          referenceId: paymentRef.id,
          referenceType: 'payment',
          entries: [
      ...
      try {
        ...
        if (customerSnap.exists()) {
          const customerData = customerSnap.data();
          const newCustomerBalance = (customerData.currentPureGoldBalance || 0) - amount;
          const newTotalPaid = (customerData.totalPureGoldPaid || 0) + amount;
      ...
      setShowPaymentDialog(false);
      setPaymentInvoiceData(null);
      setSelectedAccountType('goldInHand');
      setSelectedGoldBankId('');
      setRecordPaymentAmount('');
      setRecordPaymentNotes('');

      alert(`Payment recorded successfully!\nAmount: ${amount.toFixed(3)}g pure gold\nRemaining Balance: ${newRemainingAmount.toFixed(3)}g`);
      fetchInitialData();
  ...
  return (
    <AdminLayout>
                  ...
                  <React.Fragment key={order.id}>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex gap-2">
                        ...
                        {!order.invoiceNumber && (
                          <button
                            onClick={() => handleShowDeliveryDialog(order)}
                            className="text-blue-600 hover:text-blue-900"
                        ...
                        {order.invoiceNumber && order.paymentStatus !== 'Paid' && order.status === 'Delivered' && (
                          <button
                            onClick={async () => {
                              // Fetch invoice data
                              const invoicesPath = `${basePath}/invoices`;
                        ...
                        {/* ✅ Quick Customer Gold Payment Button */}
                        {order.remainingPureGold > 0 && (
                          <button
                            onClick={() => handleQuickCustomerPayment(order)}
                            className="text-emerald-600 hover:text-emerald-900"
                            title={`Quick Payment: ${(order.remainingPureGold).toFixed(3)}g pending`}
                          >
                            <DollarSign className="w-4 h-4" />
                          </button>
                        )}
                        ...
                        <td colSpan="8" className="bg-gray-50 p-4">
                          <div className="text-sm space-y-2">
                            ...
                            <div className="bg-yellow-50 p-3 rounded border border-yellow-200">
                              <h5 className="font-semibold text-yellow-800 mb-2">🏭 Manufacturer Gold Balance</h5>
                              <div className="space-y-1">
                                <div><strong>Gold Issued to Manufacturer:</strong> {order.goldIssued ? `${order.goldIssued.toFixed(3)}g` : 'Not issued'}</div>
                                <div><strong>Gold Returned as Jewelry:</strong> {order.goldReturned ? `${order.goldReturned.toFixed(3)}g` : 'Not delivered'}</div>
                                {order.goldIssued && order.goldReturned && (
                                  <div className={`font-semibold ${order.goldIssued - order.goldReturned === 0 ? 'text-green-600' : 'text-red-600'}`}>
                                    <strong>Balance:</strong> {(order.goldIssued - order.goldReturned).toFixed(3)}g 
                                    ({order.goldIssued - order.goldReturned === 0 ? 'Balanced' : order.goldIssued - order.goldReturned > 0 ? 'Gold Short' : 'Extra Gold Received'})
                                  </div>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="bg-blue-50 p-3 rounded border border-blue-200">
                              <h5 className="font-semibold text-blue-800 mb-2">👤 Customer Payment Status</h5>
                              <div className="space-y-1">
                                {order.invoiceId ? (
                                  <>
                                    <div><strong>Total Gold Owed:</strong> {order.totalPureGoldOwed?.toFixed(3)}g</div>
                                    <div><strong>Gold Paid:</strong> {(order.totalPureGoldOwed - (order.remainingPureGold || 0)).toFixed(3)}g</div>
                                    <div className={`font-semibold ${(order.remainingPureGold || 0) <= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                      <strong>Balance Due:</strong> {(order.remainingPureGold || 0).toFixed(3)}g 
                                      ({(order.remainingPureGold || 0) <= 0 ? 'Paid' : 'Outstanding'})
                                    </div>
                                    <div className="text-xs text-gray-600 mt-1">
                                      Invoice: {order.invoiceNumber} | Status: {order.paymentStatus || 'Not Paid'}
                                    </div>
                                  </>
                                ) : (
                                  <div className="text-gray-500 italic">Not invoiced yet</div>
                                )}
                              </div>
                            </div>
                          </div>
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            {/* Gold Balance Check */}
                            {pickupOrder.goldIssued && (
                              <div className="bg-blue-50 p-4 rounded border border-blue-200">
                                <h5 className="font-medium text-blue-900 mb-2">Gold Balance Check</h5>
                                <div className="space-y-1 text-sm">
                                  <div>Gold Issued: {pickupOrder.goldIssued.toFixed(3)}g</div>
                                  <div>Gold Returned: {finalPureGold.toFixed(3)}g</div>
                                  <div>Difference: {(finalPureGold - pickupOrder.goldIssued).toFixed(3)}g</div>
                                  <div className={`font-semibold ${finalPureGold === pickupOrder.goldIssued ? 'text-green-600' : finalPureGold > pickupOrder.goldIssued ? 'text-blue-600' : 'text-red-600'}`}>
                                    Status: {finalPureGold === pickupOrder.goldIssued ? 'Balanced' : finalPureGold > pickupOrder.goldIssued ? 'Extra Gold Received' : 'Gold Short'}
                                  </div>
                                </div>
                              </div>
                            )}
      ...
      {/* ✅ Quick Customer Payment Dialog */}
      {showQuickPaymentDialog && quickPaymentOrder && (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  <div className="text-sm text-gray-600">
                    <div className="font-medium text-gray-900">{quickPaymentOrder.customerName}</div>
                    <div>Order: {quickPaymentOrder.orderNumber}</div>
                    <div>Invoice: {quickPaymentOrder.invoiceData?.invoiceNumber || 'N/A'}</div>
                    <div className="mt-1">
                      <span className="font-medium">Remaining Balance:</span>{' '}
                      <span className="text-red-600 font-semibold">
                        {quickPaymentOrder.remainingPureGold?.toFixed(3) || quickPaymentOrder.invoiceData?.remainingPureGold?.toFixed(3) || '0.000'}g pure gold
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      step="0.01"
                      min="0"
                      value={quickPaymentData.usdAmount}
                      onChange={(e) => {
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      step="0.01"
                      min="0"
                      value={quickPaymentData.goldPrice}
                      onChange={(e) => {
                        const priceValue = e.target.value;
                        setQuickPaymentData(prev => ({
                          ...prev,
                          goldPrice: priceValue
                        }));
                      }}
                      className="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                {/* Gold Equivalent Preview */}
                {quickPaymentData.usdAmount && quickPaymentData.goldPrice && (
                  <div className="bg-blue-50 p-3 rounded-lg">
                    <div className="text-sm text-blue-800">
                      <div className="font-medium">Gold Equivalent:</div>
                      <div className="text-lg font-semibold">
                        {(parseFloat(quickPaymentData.usdAmount) / parseFloat(quickPaymentData.goldPrice)).toFixed(3)}g pure gold
                      </div>
                      {(() => {
                        const goldEquivalent = parseFloat(quickPaymentData.usdAmount) / parseFloat(quickPaymentData.goldPrice);
                        const remainingBalance = quickPaymentOrder.remainingPureGold || quickPaymentOrder.invoiceData?.remainingPureGold || 0;
                        if (goldEquivalent > remainingBalance) {
                          return (
                            <div className="text-red-600 font-medium mt-1">
                              ⚠️ Amount exceeds remaining balance by {(goldEquivalent - remainingBalance).toFixed(3)}g
                            </div>
                          );
                        }
                        return null;
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setQuickPaymentData(prev => ({
                      ...prev,
                      notes: e.target.value
                    }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    ...
                    disabled={!quickPaymentData.usdAmount || !quickPaymentData.goldPrice || 
                      parseFloat(quickPaymentData.usdAmount) <= 0 || parseFloat(quickPaymentData.goldPrice) <= 0}
                    className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed font-semibold flex items-center justify-center gap-2"
                  >
                    <DollarSign className="w-4 h-4" />
                    Record Payment
                ...
                <p className="text-xs text-gray-500 text-center">
                  This will convert USD to gold equivalent and settle customer balance.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  <div className="text-sm text-gray-600">
                    <div className="font-medium text-gray-900">{paymentInvoiceData.customerData?.name || paymentInvoiceData.customerName}</div>
                    <div>Order: {paymentInvoiceData.orderNumber}</div>
                    <div>Invoice: {paymentInvoiceData.invoiceNumber}</div>
                    <div className="mt-1">
                      <span className="font-medium">Remaining Balance:</span>{' '}
                      <span className="text-red-600 font-semibold">
                        {paymentInvoiceData.remainingPureGold?.toFixed(3) || '0.000'}g pure gold
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center">
                      <input
                        type="radio"
                        value="goldInHand"
                        checked={selectedAccountType === 'goldInHand'}
                        onChange={(e) => setSelectedAccountType(e.target.value)}
                        className="mr-2"
                      />
                      Gold in Hand (Physical gold received)
                    </label>
                    <label className="flex items-center">
                      <input
                        type="radio"
                        value="goldBank"
                        checked={selectedAccountType === 'goldBank'}
                        onChange={(e) => setSelectedAccountType(e.target.value)}
                        className="mr-2"
                      />
                      Gold Bank (Deposited to bank)
                    </label>
                  </div>
                </div>

                {/* Gold Bank Selection */}
                {selectedAccountType === 'goldBank' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Select Gold Bank *
                    </label>
                    <select
                      value={selectedGoldBankId}
                      onChange={(e) => setSelectedGoldBankId(e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
                      <option value="">Select a gold bank...</option>
                      {goldBanks.map(bank => (
                        <option key={bank.id} value={bank.id}>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Payment Amount (Pure Gold Grams) *
                  </label>
                  <div className="relative">
                    <input
                      type="number"
                  ...
                  {recordPaymentAmount && parseFloat(recordPaymentAmount) > (paymentInvoiceData.remainingPureGold || 0) && (
                    <div className="text-red-600 text-sm mt-1">
                      ⚠️ Amount exceeds remaining balance by {(parseFloat(recordPaymentAmount) - (paymentInvoiceData.remainingPureGold || 0)).toFixed(3)}g
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    ...
                    disabled={
                      !recordPaymentAmount || 
                      parseFloat(recordPaymentAmount) <= 0 || 
                      (selectedAccountType === 'goldBank' && !selectedGoldBankId) ||
                      parseFloat(recordPaymentAmount) > (paymentInvoiceData.remainingPureGold || 0)
                    }
                    className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed font-semibold flex items-center justify-center gap-2"
                  >
                    <DollarSign className="w-4 h-4" />
                ...
                <p className="text-xs text-gray-500 text-center">
                  This will record customer payment and update accounting.
                </p>
              </div>
            </div>
          </div>
