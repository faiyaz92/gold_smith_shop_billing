...
export default function Cart({ cart: propCart, isMobile, onClose }) {
  ...
  // Save cart to localStorage when propCart changes
  useEffect(() => {
    if (propCart && propCart.length >= 0) {
      setCart(propCart);
  ...
  return (
    <AnimatePresence>
      <motion.aside className="hidden lg:block basis-[30%]" initial="hidden" animate="visible" exit="exit" variants={desktopVariants}>
