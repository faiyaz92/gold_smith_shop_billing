...
export default function Products({ addToCart, removeFromCart, cart, onActiveCategoryChange }) {
        ...
        {Object.entries(productsByCategory).map(([categoryId, categoryData]) => {
          ...
          return (
            ...
            >
                ...
                {displayProducts.map((product) => {
                  ...
                  return (
                    <div key={product.id} className="bg-white rounded-lg shadow p-4 flex items-center gap-4">
                      ...
                      {quantity === 0 ? (
                        <button onClick={() => addToCart(product.name, product.discountedPrice || product.price)} className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition-colors">
                          ADD +
                        </button>
                      ) : (
                        <div className="flex items-center gap-2">
                          <button onClick={() => removeFromCart(product.name)} className="bg-gray-200 px-3 py-1 rounded hover:bg-gray-300 transition-colors">−</button>
                          <span className="min-w-[20px] text-center">{quantity}</span>
                          <button onClick={() => addToCart(product.name, product.discountedPrice || product.price)} className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 transition-colors">+</button>
                        </div>
                      )}
                    </div>
                  );
              ...
              {hasMore && (
                <button onClick={() => toggleCategoryExpansion(categoryId)} className="text-blue-500 hover:text-blue-700 text-sm font-medium">
